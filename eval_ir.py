...
def evaluate_lsa(documents: Sequence[Tuple[str, Sequence[str]]], queries: Sequence[QueryItem], k_svd: int = 200, top_k: int = 20) -> Dict[str, List[str]]:
    idx = LsaIndex(k=k_svd)
    idx.build(documents)

    run: Dict[str, List[str]] = {}
    for q in queries:
        results = idx.search(q.text, top_k=top_k)
        run[q.qid] = [doc_id for doc_id, _ in results]
    return run
...
def evaluate_sentence_transformer(
    ...
    """SentenceTransformer run (cosine via dot-product on normalized embeddings)."""
    try:
        from sentence_transformers import SentenceTransformer
    except Exception as e:
        raise RuntimeError(
            "SentenceTransformers is not installed. Install it via: pip install sentence-transformers"
        ) from e
    ...
    doc_emb = model.encode(doc_texts, show_progress_bar=True, batch_size=64, normalize_embeddings=True)
...
def evaluate_hybrid(
    tfidf_run: Dict[str, List[str]],
    st_run: Dict[str, List[str]],
    qrels: Dict[str, Iterable[str]],
    alpha: float = 0.5,
    top_k: int = 20,
) -> Tuple[Dict[str, List[str]], float]:
    """Hybridize by mixing reciprocal ranks from two runs."""
    run: Dict[str, List[str]] = {}
    ...
    for qid in qrels.keys():
        ...
        rank_a = {d: i + 1 for i, d in enumerate(a)}
        rank_b = {d: i + 1 for i, d in enumerate(b)}
        ...
        run[qid] = [d for d, _ in scored[:top_k]]

    return run, mean_average_precision(run, qrels, k=top_k)


def main() -> None:
    """Evaluate TF-IDF vs LSA vs SentenceTransformers (+ hybrid) using MAP."""
    docs_path = "data/documents.json"
    queries_path = "data/full_text_queries.json"

    raw_docs = load_eval_documents(docs_path)
    queries = load_eval_queries(queries_path)

    documents = build_documents_for_index(raw_docs)

    qrels = {q.qid: q.relevant_doc_ids for q in queries}

    ks = [10, 20, 50]

    tfidf_run = evaluate_tfidf(documents, queries, top_k=max(ks))
    lsa_run = evaluate_lsa(documents, queries, k_svd=200, top_k=max(ks))

    st_run: Optional[Dict[str, List[str]]] = None
    st_err: Optional[str] = None
    try:
        st_run = evaluate_sentence_transformer(raw_docs, queries, model_name="all-MiniLM-L6-v2", top_k=max(ks))
    except Exception as e:
        st_err = str(e)

    print(f"Queries: {len(queries)}")
    print(f"Docs (raw): {len(raw_docs)}")
    print(f"Docs (indexed after cleaning): {len(documents)}")

    for k in ks:
        print(f"MAP@{k} TF-IDF: {mean_average_precision(tfidf_run, qrels, k=k):.4f}")
        print(f"MAP@{k} LSA:    {mean_average_precision(lsa_run, qrels, k=k):.4f}")
        if st_run is not None:
            print(f"MAP@{k} ST:     {mean_average_precision(st_run, qrels, k=k):.4f}")
        else:
            print(f"MAP@{k} ST:     (skipped) {st_err}")

        if st_run is not None:
            _, hybrid_map = evaluate_hybrid(tfidf_run, st_run, qrels, alpha=0.5, top_k=k)
            print(f"MAP@{k} Hybrid (TF-IDF+ST): {hybrid_map:.4f}")
        print("-")
